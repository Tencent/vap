...
napi_value PluginRender::Play(napi_env env, napi_callback_info info)
{
    ...
    if (render) {
        ...
        if (funcArg.GetMaxArgc() >= THREE) {
            ...
            info.playDoneCallback = &Callback;
            render->ParseCallback(info.playDoneCallbackData, env, v3, render->m_id + type, type);
            render->env_ = env;
            if (info.playDoneCallbackData) {
                CallbackContext *context = (CallbackContext *)info.playDoneCallbackData;

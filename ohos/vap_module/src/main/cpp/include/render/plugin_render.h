...
public:
    ...
    static std::unordered_map<std::string, CallbackContext *> m_jsCallback; // id+type
    void ParseCallback(void *&playDoneCallbackData, napi_env env,
        napi_value val, std::string id, std::string type);
    
    void PutRef(std::string key, napi_ref ref) // type
    {
        LOGD("PutRef key %{public}s", key.c_str());
        if (refMap_.find(key) == refMap_.end()) {
            refMap_.insert(std::make_pair(key, ref));
        }
    }
    ...
    std::map<std::string, napi_ref> refMap_;
    std::vector<std::string> supportType_ = {"playDone", "stateChange", "click"};
};
